...
create table if not exists public.habits (
  ...
  interaction_type text not null default 'checkbox', -- checkbox | water | steps | moon | pills | palette | leaf | stamp | phone | jar
  target_value numeric, -- e.g. 3 (litres), 8000 (steps)
  unit text, -- e.g. 'L', 'steps'
...
create table if not exists public.journal_entries (
  ...
  content text,
  mood text, -- e.g. 'joyful' | 'calm' | 'tired' | 'stressed' | 'grateful'
...
create table if not exists public.achievements (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users (id) on delete cascade,
  type text not null, -- e.g. 'streak_7' | 'streak_14' | 'streak_30' | 'streak_100'
...
begin
  ...
  values
    ...
    (p_user_id, 'No Junk Food', '🥗', '#DFE9DF', 'leaf', null, null, 2),
    (p_user_id, 'No Sugar', '🍓', '#F6DFE1', 'stamp', null, null, 3),
    (p_user_id, 'No Packaged Food', '📦', '#F5D9C6', 'leaf', null, null, 4),
    (p_user_id, 'Drink 3 Litres Water', '💧', '#DCEBF5', 'water', 3, 'L', 5),
    (p_user_id, 'Chia Seed Water Morning', '🌱', '#DFE9DF', 'jar', null, null, 6),
    (p_user_id, 'Walk After Every Meal', '🌞', '#F6E7B2', 'steps', null, null, 7),
    (p_user_id, 'Medicines', '💊', '#E7E0F5', 'pills', null, null, 8),
    (p_user_id, 'Productive Design Practice', '🎨', '#E7E0F5', 'palette', null, null, 9),
    (p_user_id, 'No Screen After 10 PM', '📵', '#DFE9DF', 'phone', null, null, 10),
    (p_user_id, 'Sleep by 10 PM', '😴', '#E7E0F5', 'moon', null, null, 11)
